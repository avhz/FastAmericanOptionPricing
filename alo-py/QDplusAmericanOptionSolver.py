...
class QDplus:
    ...
    def price(self, tau, S):
        ...
        if self.option_type == OptionType.Put:
            pS = europ.EuropeanOption.european_put_value(
                tau, S, self.r, self.q, self.sigma, self.K
            )
            pSb = europ.EuropeanOption.european_put_value(
                tau, Sb, self.r, self.q, self.sigma, self.K
            )
        else:
            pS = europ.EuropeanOption.european_call_value(
                tau, S, self.r, self.q, self.sigma, self.K
            )
            pSb = europ.EuropeanOption.european_call_value(
                tau, Sb, self.r, self.q, self.sigma, self.K
            )
        ...
        if self.option_indicator * (Sb - S) <= 0:
            ...
            return pS + (self.K - Sb - pSb) / (
                1 - b * np.square(np.log(S / Sb)) - c * np.log(S / Sb)
            ) * np.power(S / Sb, qQD)
    ...
    def compute_miscellaneous(self, tau, S):
        ...
        if self.option_type == OptionType.Put:
            self.v_p = europ.EuropeanOption.european_put_value(
                tau, S, self.r, self.q, self.sigma, self.K
            )
        else:
            self.v_p = europ.EuropeanOption.european_call_value(
                tau, S, self.r, self.q, self.sigma, self.K
            )
        self.v_theta = europ.EuropeanOption.european_option_theta(
            tau, S, self.r, self.q, self.sigma, self.K
        )
    ...
    def exercise_boundary_func(self, S, tau):
        ...
        d1 = self.v_d1
        if self.option_type == OptionType.Call:
            ans = (1 - np.exp(-self.q * tau) * stats.norm.cdf(d1)) * S - (qQD) * (
                S - self.K - p
            )
        else:
            ans = (1 - np.exp(-self.q * tau) * stats.norm.cdf(-d1)) * S + (qQD) * (
    ...
    def c0(self, tau, S):
        ...
        return (
            ...
            * (
                1 / h
                - (theta * np.exp(self.r * tau)) / (self.r * (self.K - S - p))
                + qQDdot / (2 * qQD + N - 1)
            )
        )
    ...
    def c(self, tau, S):
        ...
        return (
            ...
            / (2 * qQD + N - 1)
            * (
                (1 - np.exp(-q * tau) * stats.norm.cdf(-d1)) / (self.K - S - p)
                + qQD / S
            )
            * dlogSdh
        )
