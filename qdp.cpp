...
BOOST_AUTO_TEST_CASE(testBaroneAdesiWhaleyValues)
{
    ...
    */
    ...
    for (auto &value : values)
    {
        ...
        ext::shared_ptr<BlackScholesMertonProcess> stochProcess(new BlackScholesMertonProcess(Handle<Quote>(spot),
    ...
    for (const auto &testCase : testCases)
    {
        for (const auto &solverType : solverTypes)
        {
            ...
            const Size maxEvaluations =
                (solverType.first == QdPlusAmericanEngine::Halley || solverType.first == QdPlusAmericanEngine::SuperHalley)
                    ? 750
...
BOOST_AUTO_TEST_CASE(testQdAmericanEngines)
{
    ...
    for (const auto &testCaseSpec : testCaseSpecs)
    {
        ...
        if ((testCaseSpec.precision > 0 && std::abs(expected - calculated) > testCaseSpec.precision) || (testCaseSpec.precision < 0 && expected > 0.1 && std::abs(calculated - expected) / expected > 0.005) || (testCaseSpec.precision < 0 && expected <= 0.1 && std::abs(expected - calculated) > 5e-4))
        {
                ...
                for (auto q : qy)
                {
                    ...
                    for (auto v : sig)
                    {
                        ...
                        for (auto s : S)
                        {
                            ...
                            spot->setValue(s);
                            numericalResults.gamma = (-gp2 + 16 * gp1 - 30 * npv + 16 * gm1 - gm2) / (12 * squared(f_g * s));
